...
def plot_mean_lines(axes_flatten,stats_df,n_models,n_measures):
    ...
    for idx in range(0,n_models):    
        for line_idx in range(0,n_measures):
            axes_flatten[idx].axhline(y=stats_df.loc[row_idx,'mean'],color=colors[line_idx])
            row_idx += 1
...
def lineplot_scores(plot_data,measures=None,iterator_dict=None,measure_labels=None,
                    x_tick_spacing='auto',y_tick_spacing='auto',
                    x_axis_title='Outer Fold',y_axis_title ='Score',
                    xlim='auto',ylim='auto',col_wrap=1,legend_title='Measures',
                    mean_lines=True,dst_dir=None,filename=None,height=4,aspect=2):
    ...
    n_models = preparation.get_number_of_models(plot_df)
    n_measures = preparation.get_number_of_measures(plot_df)
    stats_df = preparation.get_stats_df(plot_df)
    
    if iterator_dict:
        iterator_keys,iterator_combinations = preparation.get_keys_and_combos(iterator_dict)
        model_titles = preparation.get_model_titles(iterator_keys,iterator_combinations)
    
    # Plot measures as lines ####################################################
    g = sns.FacetGrid(plot_df,col='model_number',height=height,aspect=aspect,col_wrap=col_wrap)
    ...
    # FIXME: If only one measure is provided, legend is not 'outside' plot
    # FIMXE: setting legend title and labels is happening automatically
    # when providing measure_labels. Check why this error happens.
    g.add_legend()
    ...
    if measure_labels:
        if isinstance(measure_labels,list):
            ...
            legend_texts[1].set_text(measure_labels)

    ## Modify x and y-axis intervals ##########################################
    set_x_and_y_axis_layout(g,xlim,ylim,x_tick_spacing,y_tick_spacing)

    # Modify x and y-axis titles ##############################################
    # FIXME: Why is it not possible to use g.axes.flatten() iterator for setting
    # axis titles?
    for idx in range(0,n_models):
        axes_flatten[idx].set_ylabel(y_axis_title)
        axes_flatten[idx].set_xlabel(x_axis_title)
